...
`discovery-value-rejection.json` records the next discovery candidate, rejected for limited value before live A/B work. All eight final fixtures rendered identically; the earlier synthetic 640-to-183-byte result was superseded by 640-to-640. The current 44-card catalog saved 160 UTF-8 bytes (1.42%) for approximately 65 added implementation lines. These are byte counts, not measured token savings. Four calibration calls were interrupted with unknown usage; zero live worker calls launched. Source and report hashes preserve the negative result without shipping the hook.
